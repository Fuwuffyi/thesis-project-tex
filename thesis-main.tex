...
\end{figure}
Per il calcolo della riflessione speculare viene utilizzato il modello microfacet, che esprime la BRDF come:
\[
f_r(\mathbf{L}, \mathbf{V}, \mathbf{N}) = \frac{D(\mathbf{N}, \mathbf{H}) \, F(\mathbf{V}, \mathbf{H}) \, G(\mathbf{V}, \mathbf{L}, \mathbf{N})}{4 \, (\mathbf{N} \cdot \mathbf{V}) \, (\mathbf{N} \cdot \mathbf{L})}
\]
dove:
\begin{itemize}
    \item \(D\) = distribuzione dei microfacets (determina la densità di microfacets orientati verso la luce);
    \item \(F\) = termine di Fresnel (modula la riflessione speculare in base all'angolo di incidenza);
    \item \(G\) = geometria / shadowing (riduce l'illuminazione speculare dovuta a auto-occlusione);
    \item \(\mathbf{H} = \frac{\mathbf{V}+\mathbf{L}}{\|\mathbf{V}+\mathbf{L}\|}\) è il vettore a metà strada tra view e light.
\end{itemize}
