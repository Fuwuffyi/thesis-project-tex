...
\title{OpenGL e Vulkan: uno studio comparativo}
\author{Luca Palazzini}
\date{\today}
...
\begin{dedication}
   Alla mia famiglia, agli amici e ai professori, per il sostegno, la pazienza e la fiducia
...
Il presente lavoro propone lo sviluppo di un motore di rendering modulare in C++23 (progettato per operare sia con
OpenGL 4.6 che con  Vulkan 1.4) con l'obiettivo principale di condurre un confronto prestazionale e architetturale
controllato tra i due backend. \\
...
in termini di tempo medio per frame, frame rate ed utilizzo della CPU e della GPU. \\
...
utilizzando diverse configurazioni hardware, variando numero di luci e \emph{particle systems}, per misurare in modo
oggettivo i benefici derivati dal parallelismo offerto da Vulkan. \\
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=.95\linewidth]{figures/rendering_pipeline_overview.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=.95\linewidth]{figures/g_buffer_example.png}
...
nei buffer (Fig.\ref{fig:g-buffer-example} e \ref{fig:deferred-pipeline-example}), evitando di dover eseguire nuovamente le trasformazioni geometriche per ogni
...
\end{align*}
dove \(L_i\) è il contributo della luce i-esima e \(f_r\) è la funzione di Bidirectional Reflectance Distribution Function
(BRDF) che descrive come la luce viene riflessa da una superficie opaca, utilizzata per il materiale del frammento. \\
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=.9\linewidth]{figures/deferred_pipeline_example.png}
...
Il modello più diffuso nel PBR è il \emph{Cook-Torrance microfacet model}~\cite{cook1982reflectance},
utilizzato anche da Disney~\cite{burley2012pbr} e descritto in dettaglio in Pharr et al.~\cite{pharr2023pbrt},
rappresenta la superficie come un insieme di microfacets inclinati.
Ogni microfacet riflette la luce secondo la legge di Fresnel, e la distribuzione delle loro inclinazioni determina
l'aspetto speculare della superficie.
...
\end{align*}
In pratica, questo significa che, quando la luce colpisce la superficie a un angolo basso o quando la vista è laterale,
parte della luce viene "nascosta" dai microfacets, riducendo la riflessione percepita.
...
(Esempi nella Fig.\ref{fig:pbr-material-render} e \ref{fig:pbr-material-example}). \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=1.0\linewidth]{figures/pbr_material_render.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=.95\linewidth]{figures/pbr_material_example.png}
...
generare overhead significativi in applicazioni moderne e multithreaded. Le specifiche complete di OpenGL
4.6~\cite{khronos_opengl_spec} definiscono in dettaglio il comportamento dell'API e le estensioni supportate. \\
...
passaggi di rendering attraverso strutture come {\ttfamily Render Pass} e {\ttfamily Command Buffers}, e la gestione della 
sincronizzazione è completamente esplicita mediante {\ttfamily Semaphore} e {\ttfamily Fence}. Questo approccio riduce
...
\begin{itemize}
   ...
   \item \textbf{ASSIMP}: parsing dei modelli 3D in formato {\ttfamily .obj} e {\ttfamily .fbx}.
\end{itemize}
...
Nel backend Vulkan, il motore deve supportare la generazione multithread dei {\ttfamily command buffer}, così da sfruttare
...
Dal punto di vista hardware, i test sono stati condotti su molteplici configurazioni dotate di processore multi-core e GPU
...
La scena è organizzata tramite un {\ttfamily Scene Manager}, che contiene nodi ({\ttfamily Node}) e componenti specifici come
{\ttfamily TransformComponent}, {\ttfamily LightComponent} e {\ttfamily ParticleSystemComponent}. Ogni nodo può avere figli,
permettendo di costruire gerarchie complesse e di propagare trasformazioni e proprietà luminose lungo la scena. \\
Il modulo di rendering interagisce con i backend grafici attraverso l'interfaccia astratta {\ttfamily IRenderer}, che
definisce i metodi fondamentali per inizializzare le pipeline, eseguire il rendering dei frame e liberare le risorse. Le
implementazioni concrete {\ttfamily GLRenderer} e {\ttfamily VulkanRenderer} incapsulano la complessità specifica delle due
API, permettendo al resto del sistema di interagire in maniera unificata (Fig.\ref{fig:uml-activity-dynamic-backend}). \\
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/uml_activity_dynamic_backend.png}
    \caption{Selezione dinamica del backend grafico all'avvio.}
    \label{fig:uml-activity-dynamic-backend}
\end{figure}
La gestione delle risorse è centralizzata nel modulo {\ttfamily ResourceManager}, che si occupa di creare, caricare e
...
{\ttfamily PerformanceLogger} e rese disponibili tramite {\ttfamily PerformanceGUI}, consentendo il monitoraggio in tempo reale. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=1.0\linewidth]{figures/uml_scene_renderer.png}
...
L'interfaccia {\ttfamily IRenderer} definisce il contratto per le operazioni di rendering, mentre {\ttfamily GLRenderer} e
{\ttfamily VulkanRenderer} forniscono le implementazioni dettagliate. Questa architettura consente di cambiare backend
senza modificare la struttura dei componenti della scena. \\
Le classi legate alla scena, tra cui {\ttfamily Node} e {\ttfamily TransformComponent}, sono progettate per propagare le trasformazioni
lungo gerarchie complesse, permettendo operazioni come parenting, rotazioni locali e scaling globale. \\
I materiali PBR sono gestiti tramite {\ttfamily MaterialTemplate} e {\ttfamily MaterialInstance}, separando i dati statici
...
esplicito di buffer, command pool e sincronizzazione tramite {\ttfamily Semaphore} e {\ttfamily Fence}. Questa separazione facilita
...
\begin{itemize}
    \item {\ttfamily gAlbedo}: canali RGB per il colore diffuso, canale A per l'AO;
    \item {\ttfamily gNormal}: normali codificate tramite un algoritmo di \emph{oct-encoding} nei canali RG, canale B per roughness, canale A per metallic;
    \item gDepth: depth texture utilizzata per ottenere la posizione dei mesh per pixel.
\end{itemize}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/g_buffer_textures.png}
...
In Vulkan, la pipeline è progettata per sfruttare il multithreading della CPU. I {\ttfamily command buffer} sono generati
in parallelo su più thread, ciascuno occupandosi di un sottoinsieme di oggetti della scena, mentre la sincronizzazione
è gestita tramite {\ttfamily Semaphore} e {\ttfamily Fence}. Questo approccio riduce l'overhead del driver e consente di
scalare le prestazioni con il numero di core disponibili (Fig.\ref{fig:uml-activity-geometrythreading}). In OpenGL, invece,
la pipeline rimane single-thread, affidando la gestione dei comandi al driver, il che evidenzia le differenze prestazionali
tra le due API in scenari complessi. \\
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.95\linewidth]{figures/uml_activity_geometrythreading.png}
    \caption{Generazione di command buffer in parallelo.}
    \label{fig:uml-activity-geometrythreading}
\end{figure}

\section{Gestione delle Risorse}
La gestione delle risorse è centralizzata nel modulo {\ttfamily ResourceManager}, che crea, carica e memorizza mesh e
texture. Le factory astratte ({\ttfamily IResourceFactory}) permettono di implementare varianti specifiche per OpenGL e Vulkan.
...
Ogni risorsa ha un {\ttfamily ResourceHandle} che garantisce riferimenti unici tramite UID (\emph{Unique Identifier}). \\
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.95\linewidth]{figures/uml_resources.png}
...
\begin{itemize}
    \item {\ttfamily src/core}: codice indipendente dall'API, inclusi gestione della scena, materiali e componenti;
    \item {\ttfamily src/gl} e {\ttfamily src/vk}: implementazioni specifiche dei renderer e costrutti API specific;
    \item {\ttfamily resources/shaders/gl} e {\ttfamily resources/shaders/vk}: shader GLSL per ciascun backend;
    \item {\ttfamily resources/meshes} e {\ttfamily resources/textures}: asset 3D e texture utilizzate dal motore.
\end{itemize}
La selezione del backend avviene a runtime tramite opzioni di comando, senza alterare il flusso logico
del motore~\ref{lst:renderer-factory}. Questo permette di eseguire test comparativi tra OpenGL e Vulkan utilizzando la stessa
scena e gli stessi asset.
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:renderer-factory},caption={Factory pattern per la creazione dinamica del renderer in base all'API selezionata.}]{listings/renderer_factory.cpp}

\section{Componenti Principali}
Il cuore del motore è costituito dai seguenti moduli:
\begin{itemize}
    \item {\ttfamily IRenderer}: interfaccia astratta che definisce le operazioni principali di inizializzazione,
          rendering e pulizia delle risorse.
    \item {\ttfamily GLRenderer} e {\ttfamily VulkanRenderer}: implementazioni specifiche per ciascun backend,
          incapsulando le complessità della gestione di buffer, pipeline e sincronizzazione.
    \item {\ttfamily Scene} e {\ttfamily Node}: organizzano gli oggetti in una gerarchia e gestiscono le trasformazioni.
    \item {\ttfamily ResourceManager}: carica, memorizza e gestisce mesh, texture e shader, utilizzando factory
          specifiche per OpenGL e Vulkan.
\end{itemize}

\section{Gestione della Scena e dei Componenti}
La scena è rappresentata come un grafo di nodi ({\ttfamily Node}) che possono contenere componenti
come {\ttfamily TransformComponent}, {\ttfamily LightComponent} e {\ttfamily ParticleSystemComponent} (Listino ~\ref{lst:scene-management}).
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:scene-management},caption={Struttura della classe Node per la gestione gerarchica della scena.}]{listings/scene_management.cpp}
...
aggiorna i componenti registrati. I {\ttfamily TransformComponent} calcolano le matrici globali a
...
di rendering (Listino ~\ref{lst:scene-update}).
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:scene-update},caption={Logica di aggiornamento della scena con propagazione delle trasformazioni e update dei componenti.}]{listings/scene_update.cpp}

\section{Gestione delle risorse}
La gestione delle risorse è centralizzata nel modulo {\ttfamily ResourceManager}, che si occupa di
caricare, creare e memorizzare mesh, texture e shader. Ogni risorsa è identificata tramite
{\ttfamily ResourceHandle} con UID univoci, garantendo riferimenti consistenti senza duplicazioni (Listino ~\ref{lst:resource-management}).
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:resource-management},caption={Registrazione e gestione centralizzata delle risorse tramite ResourceHandle con UID univoci.}]{listings/resource_management.cpp}
Le factory astratte ({\ttfamily IResourceFactory}) permettono di istanziare risorse specifiche
...
Ogni materiale è rappresentato come combinazione di {\ttfamily MaterialTemplate} e
{\ttfamily MaterialInstance}, separando dati statici da parametri runtime. I template definiscono
shader, textures e valori di default, mentre le istanze permettono variazioni specifiche
per ciascun oggetto.
Ogni materiale PBR fa riferimento a un {\ttfamily MaterialTemplate} e a una o più
{\ttfamily MaterialInstance}, consentendo di parametrizzare roughness, metalness e albedo senza
...
canali non utilizzati e l'AO è inserita nell'alpha dell'albedo. L’oct-encoding consente di comprimere una normale
tridimensionale in due componenti, riducendo l’occupazione di memoria nel G-buffer. Nel codice,
la normale \( n \) viene prima proiettata sul piano unitario dividendo ciascun asse per la somma dei valori assoluti
delle sue componenti. Se la componente \( z \) è negativa, le coordinate vengono riflesse per mappare l’emisfero inferiore
sul superiore, garantendo continuità. Le coordinate risultanti sono poi scalate da \([-1,1]\) a \([0,1]\) per la
memorizzazione in texture a 8 bit per canale. Questo metodo permette di memorizzare normali con buona precisione
usando solo due canali (RG), liberando spazio per altri parametri PBR nello stesso G-buffer (Listino ~\ref{lst:geometry-pass-snippet}). \\
\lstinputlisting[float,floatplacement=htbp,language=GLSL,label={lst:geometry-pass-snippet},caption={Shader GLSL del geometry pass con oct-encoding delle normali e packing ottimizzato dei parametri PBR.}]{listings/geometry_snippet.glsl}
...
ridondanti (Listino ~\ref{lst:lighting-pass-snippet}).
\lstinputlisting[float,floatplacement=htbp,language=GLSL,label={lst:lighting-pass-snippet},caption={Shader GLSL del lighting pass con implementazione del modello Cook-Torrance PBR.}]{listings/lighting_snippet.glsl}
Il motore implementa un sistema di materiali PBR basato sul modello Cook-Torrance. Ogni materiale è descritto
tramite {\ttfamily MaterialTemplate} e {\ttfamily MaterialInstance}, con parametri fisicamente plausibili come
...
Il motore integra sistemi di particelle configurabili tramite {\ttfamily ParticleSystemCompo-\\nent}.
Ogni sistema contiene array di {\ttfamily ParticleInstanceData} che memorizzano posizione, velocità,
colore e altri attributi. Le particelle sono aggiornate per ogni frame tramite integrazione
...
\section{Interfaccia di Debug e Monitoraggio}
Il motore integra un sistema completo di interfaccia grafica basato su ImGui~\cite{imgui}, che fornisce strumenti di
debug, monitoraggio delle prestazioni e un editor visuale per la scena e i materiali. L’interfaccia è progettata per
essere non invasiva durante il rendering e consente l’accesso in tempo reale a tutti i parametri critici del motore. \\
Il modulo {\ttfamily PerformanceGUI} implementa un overlay sempre visibile che mostra le principali metriche di
performance (Fig.~\ref{fig:performance-overlay}). Attraverso questo pannello è possibile monitorare il frame rate
corrente e medio, oltre al tempo di frame in millisecondi, con valori minimo, massimo e medio. Vengono inoltre mostrati
l’utilizzo della CPU e della GPU sia in percentuale sia in tempo assoluto, l’occupazione della memoria video e di
sistema rappresentata con barre di progresso colorate, il conteggio delle risorse caricate e la memoria utilizzata
dal {\ttfamily ResourceManager}. Un grafico temporale degli ultimi 1024 frame time consente di individuare spike o
instabilità, mentre un breakdown dettagliato mostra i tempi di ciascuna render pass (geometry, lighting, particles,
gizmos e ImGui). \\
\begin{figure}[htbp]
   \centering
   \fbox{\includegraphics[width=0.5\linewidth]{figures/performance_overlay.png}}
   \caption{Performance overlay che mostra metriche in tempo reale e grafico dei frame time.}
   \label{fig:performance-overlay}
\end{figure}
L’implementazione si basa su un sistema di accumulo statistico che conserva valori medi, minimi e massimi nel tempo,
consentendo analisi più accurate delle prestazioni. Il codice è ottimizzato per ridurre l’overhead della GUI stessa,
grazie all’uso di {\ttfamily std::ranges} e operazioni a costo costante dove possibile (Listino~\ref{lst:performance-gui-snippet}). \\
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:performance-gui-snippet},caption={Implementazione del performance overlay con statistiche in tempo reale.}]{listings/performance_gui_snippet.cpp}
Un altro elemento chiave dell’interfaccia è il {\ttfamily Scene Inspector}, che offre una vista gerarchica completa di tutti i nodi
della scena e consente la selezione, l’ispezione e la modifica dei componenti in tempo reale (Fig.~\ref{fig:scene-inspector}).
L’interfaccia è organizzata in due pannelli principali: la \textit{Hierarchy View}, che mostra l’albero dei nodi con
indicatori di stato e funzioni di espansione o collasso, e l’\textit{Inspector Panel}, che presenta i dettagli del nodo
selezionato e permette di modificare i componenti associati, come Transform, Light, Renderer o ParticleSystem. L’inspector
consente anche l’aggiunta dinamica di nuovi nodi e componenti tramite menu contestuali, semplificando la prototipazione
rapida e il debug visuale. La traversata ricorsiva ottimizzata della gerarchia assicura coerenza tra la vista logica e
la rappresentazione visuale. \\
\begin{figure}[htbp]
   \centering
   \fbox{\includegraphics[width=0.7\linewidth]{figures/scene_inspector.png}}
   \caption{Scene inspector con vista gerarchica dei nodi e pannello di ispezione dei componenti.}
   \label{fig:scene-inspector}
\end{figure}
Il {\ttfamily MaterialEditor} fornisce invece un’interfaccia completa per la gestione e la modifica dei materiali
PBR (Fig.~\ref{fig:material-editor}). È composto da tre finestre principali: il \textit{Material Browser}, che elenca
tutti i materiali disponibili e supporta la creazione e la selezione tramite drag-and-drop; il pannello
\textit{Material Properties}, che consente di modificare i parametri del materiale selezionato come roughness,
metalness o albedo, utilizzando controlli dedicati per ogni tipo di dato (slider, color picker o input numerici);
e infine la sezione \textit{Texture Slots}, che gestisce le texture associate al materiale con anteprima e tooltip informativi.
Il sistema di drag-and-drop permette di trascinare materiali o texture direttamente sui componenti della scena, con
aggiornamento immediato del rendering grazie al refresh degli uniform buffer (Listino~\ref{lst:material-editor-snippet}). \\
\begin{figure}[htbp]
   \centering
   \fbox{\includegraphics[width=0.9\linewidth]{figures/material_editor.png}}
   \caption{Material editor con browser, properties panel e texture slot editor.}
   \label{fig:material-editor}
\end{figure}
\lstinputlisting[float,floatplacement=htbp,language=C++,label={lst:material-editor-snippet},caption={Sistema di drag-and-drop per texture nel material editor.}]{listings/material_editor_snippet.cpp}
Per la gestione delle risorse grafiche, il {\ttfamily Texture Browser} offre una galleria visuale di tutte le texture
caricate, disposte in una griglia adattiva (Fig.~\ref{fig:texture-browser}). Ogni elemento mostra un’anteprima ad alta
risoluzione, il nome completo del file e le dimensioni in pixel, oltre al supporto per il drag-and-drop verso gli slot
dei materiali. L’implementazione astrae le differenze tra i backend OpenGL e Vulkan, convertendo gli identificativi GPU
in {\ttfamily ImTextureID} compatibili con il sistema di rendering immediato di ImGui. \\
\begin{figure}[htbp]
   \centering
   \fbox{\includegraphics[width=0.9\linewidth]{figures/texture_browser.png}}
   \caption{Texture browser con galleria visuale e informazioni sulle risorse.}
   \label{fig:texture-browser}
\end{figure}

\section{Differenze di Implementazione tra OpenGL e Vulkan}
L'approccio low-level di Vulkan, descritto nelle specifiche ufficiali~\cite{khronos_vulkan_spec} e approfondito nei
tutorial della community~\cite{vulkan_tutorial,vkguide}, richiede una comprensione più profonda dell'architettura GPU
rispetto all'astrazione offerta da OpenGL~\cite{khronos_opengl_spec}. \\
...
nelle classi {\ttfamily GLRenderer} e {\ttfamily VulkanRenderer}. Questo approccio garantisce la
...
{\ttfamily SPIR-V}, garantendo che il codice eseguito dal driver sia deterministico e portabile.
...
Un esempio della compilazione in SPIR-V eseguita dal file CMake viene mostrato nel listino~\ref{lst:spv-compilation}.
\lstinputlisting[float,floatplacement=htbp,label={lst:spv-compilation},caption={Script CMake per la compilazione automatica degli shader GLSL in formato SPIR-V per Vulkan.}]{listings/spv_compilation.cmake}
...
la scena, i materiali PBR e le geometrie. Esempio di una scena (Fig.\ref{fig:window-screenshot}). Le scene sono caricate
dinamicamente secondo le configurazioni riportate nella Tabella~\ref{tab:table-scenes}.
\begin{table}[h!]
\centering
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=.95\linewidth]{figures/window_screenshot.png}
   \caption{Render vulkan della scena 2 con UI di debug.}
   \label{fig:window-screenshot}
\end{figure}
Le misurazioni raccolte riguardano tempo medio per frame, varianza del frame time, tempo richiesto per ciascuna
render pass, overhead e utilizzo della CPU. Sono stati impiegati strumenti integrati nel motore ({\ttfamily GLTimer} e
{\ttfamily VulkanGPUTimer}) e strumenti esterni di profiling per confermare i risultati. I test sono stati eseguiti su
...
\begin{tabular}{l|l|l|l}
   \textbf{ID} & \textbf{CPU} & \textbf{GPU} & \textbf{VRAM} \\ \hline
   A & AMD Ryzen 7 9800X3D & NVIDIA RTX 5080 & 16 GB GDDR7 \\ % Fuwuffy
   B & AMD Ryzen 7 2700 & NVIDIA RTX 3070ti & 8 GB GDDR6X \\ % Ponse
   C & Intel Core i7-8700K & NVIDIA RTX 3070 & 8 GB GDDR6 \\ % Medijaster
   D & AMD Ryzen 7 8700F & NVIDIA GT 1030 & 4 GB GDDR5 \\ % FuwuffyParents
   E & Intel Core i3-1005G1 & Intel UHD Graphics (GT1) & X \\ % FuwuffyLaptop
\end{tabular}
...
\section{Risultati}
I risultati mostrano un miglioramento complessivo delle prestazioni del motore di rendering quando viene
utilizzata l'API Vulkan rispetto a OpenGL, mantenendo invariate le condizioni di test e le scene analizzate.
Per fornire una visione dettagliata delle prestazioni, le Figure~\ref{fig:fps-by-hw-1}
e~\ref{fig:fps-by-hw-2} mostrano il confronto diretto tra OpenGL e Vulkan per ciascuna scena,
in funzione della configurazione hardware utilizzata. Questi grafici evidenziano come il vantaggio
prestazionale di Vulkan vari significativamente in base alla complessità della scena e alle
caratteristiche dell'hardware. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/fps_by_hardware_scenes_0_1_2.png}
   \caption{Confronto FPS tra OpenGL e Vulkan per le scene 0, 1 e 2 su diverse configurazioni hardware.}
   \label{fig:fps-by-hw-1}
\end{figure}
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/fps_by_hardware_scenes_3_4.png}
   \caption{Confronto FPS tra OpenGL e Vulkan per le scene 3 e 4 su diverse configurazioni hardware.}
   \label{fig:fps-by-hw-2}
\end{figure}
La Figura~\ref{fig:heatmap-pc-fps} riassume il numero medio di frame per secondo (FPS) ottenuto da ciascuna
configurazione hardware su tutte le scene e per entrambe le API grafiche. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/heatmap_pc_fps.png}
   \caption{FPS medi registrati per ogni configurazione hardware e scena, confrontando Vulkan e OpenGL.}
   \label{fig:heatmap-pc-fps}
\end{figure}
Nelle scene con carichi di lavoro medio-alti per la CPU (scene 0, 1, 2 e 4) è stato osservato un incremento medio
delle prestazioni compreso tra il 20\% e il 25\%. Tale miglioramento si manifesta principalmente attraverso
una riduzione del tempo medio per frame, indicativa di un minore overhead lato CPU con Vulkan. \\
La scena 3, caratterizzata dal più alto numero di sorgenti luminose, da una maggiore complessità di shading e
dall'assenza di sistemi particellari, ha evidenziato un decremento di performance moderato, compreso tra il 5\% e il 15\%.
In questo caso, la natura multithreaded del command recording di Vulkan offre un vantaggio ridotto, poiché il
carico sulla CPU risulta inferiore. Inoltre, parte della differenza prestazionale può essere attribuita
all'implementazione meno ottimizzata del backend Vulkan rispetto a quello OpenGL, dovuta alla maggiore
complessità dell'API e alla sua relativa novità nel contesto di sviluppo del progetto. \\
Nonostante ciò, sull'architettura hardware C è stato registrato un incremento di prestazioni particolarmente
rilevante nella scena 3, pari a circa il 250\%, suggerendo che il comportamento di Vulkan beneficia in modo
più marcato di determinate combinazioni CPU-GPU e driver.
Inoltre, in tutte le scene si è riscontrata una riduzione dell'utilizzo della memoria video (VRAM) con Vulkan, segno di
una gestione più efficiente delle risorse grafiche e dell'allocazione esplicita della memoria rispetto all'approccio
automatizzato di OpenGL. \\
Le Figure~\ref{fig:gain-by-hw-1} e~\ref{fig:gain-by-hw-2} illustrano il guadagno percentuale
ottenuto con Vulkan rispetto a OpenGL per ciascuna combinazione di scena e hardware. I valori
positivi indicano un miglioramento prestazionale, mentre valori negativi evidenziano situazioni
in cui OpenGL risulta più performante. L'analisi mostra come il beneficio di Vulkan sia più
marcato nelle scene CPU-bound e su hardware con maggiore parallelismo. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/gain_by_hardware_scenes_0_1_2.png}
   \caption{Guadagno percentuale di Vulkan rispetto a OpenGL per le scene 0, 1 e 2.}
   \label{fig:gain-by-hw-1}
\end{figure}
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/gain_by_hardware_scenes_3_4.png}
   \caption{Guadagno percentuale di Vulkan rispetto a OpenGL per le scene 3 e 4.}
   \label{fig:gain-by-hw-2}
\end{figure}
La Figura~\ref{fig:heatmap-pc-improvements} mostra i miglioramenti percentuali ottenuti, suddivisi per configurazione
hardware e scena, fornendo una visione comparativa del guadagno effettivo per API. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/heatmap_pc_improvements.png}
   \caption{Miglioramento percentuale delle prestazioni per ogni configurazione hardware e scena,
   calcolato come differenza relativa tra Vulkan e OpenGL.}
   \label{fig:heatmap-pc-improvements}
\end{figure}

\section{Discussione}
I risultati sperimentali confermano il vantaggio architetturale offerto da Vulkan nei contesti in cui la CPU rappresenta
il principale collo di bottiglia. L'approccio esplicito dell'API e la possibilità di parallelizzare la registrazione
dei comandi consentono un utilizzo più efficiente dei core disponibili, riducendo sensibilmente l'overhead di gestione
del driver rispetto a OpenGL. \\
Nei casi di carico medio (scene 0, 1 e 2), caratterizzati da un numero moderato di luci dinamiche e materiali PBR,
si osserva un incremento prestazionale compreso tra il 20\% e il 25\%. In questi scenari il carico della GPU rimane
sufficiente a mantenere la pipeline occupata, ma l'efficienza del multithreading di Vulkan permette di ridurre la
latenza nella preparazione delle draw call. Ciò si traduce in una maggiore stabilità visiva e in un comportamento più
prevedibile del motore di rendering. Tuttavia, in alcune configurazioni Vulkan mostra una maggiore variabilità degli FPS
rispetto alla media. Questo comportamento è attribuibile al fatto che l'API registra i command buffer del frame
successivo mentre la GPU sta ancora elaborando quello corrente, introducendo leggere oscillazioni nel frame time dovute alla
concorrenza tra CPU e GPU (Fig.\ref{fig:frametime-stability}). \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/frametime_stability.png}
   \caption{Stabilitá degli FPS per scena.}
   \label{fig:frametime-stability}
\end{figure}
La scena 3, con il numero massimo di sorgenti luminose e un'elevata complessità di shading, presenta un comportamento
più eterogeneo. Su alcune configurazioni hardware (in particolare la piattaforma C) è stato registrato un incremento
eccezionale delle prestazioni, fino al 250\%, mentre su altre si è osservato un lieve peggioramento (5-15\%). Questa
divergenza è attribuibile principalmente a due fattori: la diversa ottimizzazione dei driver e il grado di maturità
dell'implementazione Vulkan rispetto a quella OpenGL. Poiché il carico in questo scenario è prevalentemente GPU-bound,
l'impatto del command recording multithreaded risulta ridotto, mentre il peso della gestione manuale delle pipeline e
delle risorse di memoria può introdurre un overhead aggiuntivo nei casi meno ottimizzati.
La scena 4, focalizzata sui sistemi particellari e fortemente legata all'elaborazione GPU, mostra un guadagno limitato
ma costante. In questo contesto la riduzione dell'overhead CPU incide in misura marginale, mentre il tempo di frame è
determinato principalmente dalle capacità computazionali della GPU. Ciò conferma che i benefici di Vulkan si manifestano
più chiaramente nei casi in cui l'applicazione è vincolata dalle prestazioni della CPU. \\
Nel complesso, Vulkan si dimostra un'API più scalabile, prevedibile e capace di sfruttare in modo più efficiente
l'hardware moderno, specialmente in presenza di architetture multicore e scene complesse. OpenGL, pur mantenendo buone
prestazioni in contesti moderati, tende a saturare rapidamente le risorse CPU a causa del vincolo al thread principale
e della maggiore astrazione offerta dal driver. Qui vengono riportati i risultati ottenuti su tutte le scene da tutte le
configurazioni hardware (Fig.\ref{fig:cross-pc-results}). \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/cross_pc_results.png}
   \caption{FPS ottenuti da tutti gli hardware su tutte le scene.}
   \label{fig:cross-pc-results}
\end{figure}
Per una valutazione più dettagliata della stabilità delle prestazioni, sono stati confrontati i valori di FPS relativi
ai casi migliore e peggiore per entrambe le API. Come evidenziato in Figura~\ref{fig:percentile-comparisons}, Vulkan
mostra una stabilità comparabile a quella di OpenGL nella maggior parte degli scenari. Tuttavia, la scena 3 presenta
una variabilità significativa degli FPS rispetto a OpenGL, suggerendo che in questo caso specifico il comportamento può
essere più sensibile alla configurazione hardware. Ciò potrebbe spiegare le discrepanze osservate nei test eseguiti
sulla piattaforma D. \\
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{figures/percentile_comparison.png}
   \caption{Confronto dei valori di FPS tra il caso migliore e il caso peggiore.}
   \label{fig:percentile-comparisons}
\end{figure}
È tuttavia importante evidenziare che questi vantaggi prestazionali sono ottenuti al costo di una maggiore complessità
di sviluppo. Vulkan richiede la gestione esplicita di pipeline, sincronizzazioni e allocazioni di memoria, aspetti che
in OpenGL sono automatizzati dal driver. Questo si traduce in un incremento del tempo di sviluppo e in una maggiore
attenzione necessaria per evitare errori di gestione. In cambio, l'applicazione ottiene un controllo fine sull'intero
flusso di rendering e la possibilità di ottimizzare il comportamento in funzione dell'hardware e del carico di lavoro.
In sintesi, i dati sperimentali confermano che Vulkan, pur imponendo una curva di apprendimento più ripida e una
maggiore complessità ingegneristica, offre vantaggi concreti in termini di efficienza, parallelismo e prevedibilità
del comportamento. OpenGL resta più immediato da implementare, ma meno adatto a sfruttare pienamente le architetture
multicore e le GPU di nuova generazione.
...
Il lavoro ha portato alla realizzazione di un motore di rendering sviluppato in C++23, in grado di operare
sia con l'API OpenGL 4.6 sia con Vulkan 1.4, selezionabili tramite parametro a riga di comando.
Il motore implementa un sistema di rendering differito (geometry e lighting pass), con supporto a materiali PBR,
luci direzionali, spot e point light, oltre a sistemi particellari basati su instancing e una pipeline dedicata
al rendering dei gizmo di debug.
L'architettura è stata progettata in modo modulare e orientato agli oggetti, facendo uso di librerie moderne
come GLFW, GLAD, Vulkan Memory Allocator, VkBootstrap e ImGui.
L'obiettivo principale del lavoro è stato quello di confrontare direttamente le prestazioni e il comportamento
delle due API grafiche, con particolare attenzione all'impatto del multithreading di Vulkan rispetto all'approccio
single-threaded di OpenGL. Il motore è stato realizzato per consentire test riproducibili, garantendo una
valutazione oggettiva e indipendente dalle condizioni esterne.

\paragraph{Risultati Principali}
I risultati sperimentali hanno evidenziato un miglioramento medio delle prestazioni compreso tra il 20\% e il 25\%
a favore di Vulkan nelle scene a carico grafico medio, e fino al 250\% in specifici casi ad alta complessità,
come le scene con numerose sorgenti luminose.
Le analisi confermano che Vulkan sfrutta in modo più efficiente la CPU grazie alla possibilità di generare e inviare
i comandi di rendering in parallelo su più thread, riducendo l'overhead del driver.
Ne consegue un utilizzo più costante della GPU, con minori tempi di inattività e una maggiore regolarità del frame time.
Al contrario, in OpenGL il vincolo al thread principale introduce un collo di bottiglia lato CPU, limitando la
scalabilità in presenza di carichi elevati. \\
Oltre ai benefici prestazionali, Vulkan ha mostrato una maggiore prevedibilità e stabilità delle performance,
specialmente all'aumentare del numero di draw call, confermando la validità del suo approccio a basso livello
e la sua idoneità per le architetture multicore moderne.

\paragraph{Limitazioni del Lavoro}
Nonostante i risultati ottenuti, il lavoro presenta alcune limitazioni.
La complessità intrinseca di Vulkan comporta un tempo di sviluppo maggiore e una curva di apprendimento più ripida
rispetto a OpenGL. La gestione manuale di pipeline, sincronizzazione e memoria aumenta la possibilità di errori
e richiede una progettazione più attenta.
Inoltre, i test sono stati condotti su un numero limitato di configurazioni hardware e non includono un'analisi
approfondita del comportamento su GPU integrate o dispositivi mobili. Alcune ottimizzazioni specifiche per GPU AMD
e Intel potrebbero portare a risultati differenti. \\
Infine, il motore, pur completo per gli scopi di questa tesi, non include ancora tecniche avanzate come compute shader
per la gestione delle particelle o sistemi di post-processing complessi, che potrebbero ulteriormente ampliare
le differenze prestazionali tra le API.

\paragraph{Sviluppi Futuri}
Gli sviluppi futuri del progetto possono procedere in diverse direzioni.
Dal punto di vista tecnico, l'integrazione di un renderer basato su compute shader e la gestione asincrona dei dati
tra CPU e GPU permetterebbero di analizzare in modo più approfondito i vantaggi del modello esplicito di Vulkan.
L'introduzione di un job system dedicato al caricamento e all'aggiornamento delle risorse consentirebbe di estendere
la gestione multithreaded del motore e migliorare la scalabilità complessiva.
Un'ulteriore evoluzione potrebbe riguardare la realizzazione di un sistema di rendering ibrido, capace di utilizzare
OpenGL o Vulkan in base all'hardware disponibile, massimizzando compatibilità e portabilità.
Infine, il motore potrebbe essere ampliato con funzionalità avanzate, come il supporto al ray tracing,
tecniche di global illumination, asset streaming, animazioni scheletriche e gestione di armature,
rendendolo una base solida per progetti di ricerca o applicazioni real-time di livello professionale.
...
\begin{acknowledgements}
   Desidero ringraziare la \textbf{Prof.ssa~[Damiana Lazzaro]}, relatrice della tesi, per la disponibilità, la guida e
   i suggerimenti forniti durante le diverse fasi del lavoro. \\
   Il lavoro si è basato sull'utilizzo di diverse librerie e strumenti open-source, tra cui
   \lstinline{GLFW}~\cite{glfw}, \lstinline{GLAD}~\cite{glad}, \lstinline{Vulkan}~\cite{vulkan_overview,khronos_vulkan_spec},
   \lstinline{VkBootstrap}~\cite{vkbootstrap}, \lstinline{Vulkan Memory Allocator}~\cite{vma} e \lstinline{ImGui}~\cite{imgui}
   che hanno reso possibile la realizzazione del motore di rendering. \\
   Un ringraziamento particolare è rivolto alla comunità open-source per il continuo contributo allo sviluppo di
   strumenti e risorse che rendono accessibile la ricerca e la sperimentazione nel campo della computer graphics.
